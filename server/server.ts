...
server.use("/save-token", async (req: Request, res: Response) => {
  try {
    const { owner, fileId, tokenAddress, tokenFactory } = req.body;
    ...
    if (!tokenExists) {
      ...
      res
        .status(201)
        .json({ message: "Token information saved", createdToken });
      return;
    }

    tokenExists.fileId.push(fileId);
    tokenExists.tokenAddress.push(tokenAddress);
    tokenExists.tokenFactory.push(tokenFactory);
    tokenExists.save();

    res.status(200).json({ message: "Token saved successfully", tokenExists });
  } catch (error) {
    console.error(error);
